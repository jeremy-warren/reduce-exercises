...
as the number of times the vowel appears in the string. This function should be case insensitive so a lowercase letter and 
...
function vowelCount(str) {
    const vowels = 'aeiou';
    const stringSplitter = (str) => str.toLowerCase().split('')
    const vowelCheck = (letter) => vowels.indexOf(letter) != -1;

    const splitString = stringSplitter(str)

    return splitString.reduce(function(existing, next){
        if (vowelCheck(next)){
            existing[next]?  existing[next] ++ : existing[next] = 1;
        }
        return existing;
    },{});
}
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function (acc, next, index){ //need to ask mentor about index stuff
...
function partition(arr, callback) { //looked at the solution and it's definitely more elegant, but I like my falseBoxes.
    const trueBox = [];
    const falseBox = [];
    const returnArray = [trueBox,falseBox];
    const theNothing = arr.reduce(function (acc, current){
        callback(current)? trueBox.push(current) : falseBox.push(current);
        return acc;
    },arr)
